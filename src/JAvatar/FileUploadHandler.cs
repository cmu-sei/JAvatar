        ...
        )
        {
            ...
            while (section != null)
            {
                ...
                if (hasContentDispositionHeader)
                {
                    ...
                    {
                        metadata.Add("filename", HeaderUtilities.RemoveQuotes(contentDisposition.FileName).Value);
                        if (!metadata["size"].HasValue()) metadata.Add("size", "1000000000");
                        if (!metadata["upload-key"].HasValue()) metadata.Add("upload-key", Guid.NewGuid().ToString());
                        Stream targetStream = getDestinationStream.Invoke(metadata);
                        FileUploadStatus status = new FileUploadStatus
                        {
                            Metadata = metadata,
                            Key = metadata["upload-key"],
                            Size = Int64.Parse(metadata["size"])
                        };
    ...
    public class FileUploadStatus
    {
        ...
        public int Rate {
            get
            {
                ...
                return Convert.ToInt32(Count / (now.Subtract(StartedAt).TotalSeconds + 1));
            }
        }
    }
}
